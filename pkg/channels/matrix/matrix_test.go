...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("permaclaw")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@permaclaw hello", want: true},
		{text: "hi @permaclaw:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bpermaclaw\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@permaclaw:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@permaclaw:matrix.org hello", want: "hello"},
		{in: "@permaclaw, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@permaclaw:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@permaclaw:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@permaclaw:matrix.org">PermaClaw</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40permaclaw%3Amatrix.org">PermaClaw</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@permaclaw:matrix.test"), "")
	if err != nil {
