...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via PERMACLAW_KEY_PASSPHRASE
...
//  2. PERMACLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/permaclaw_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "PERMACLAW_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading PERMACLAW_KEY_PASSPHRASE from the
...
const (
	fileScheme = "file://"
	encScheme  = "enc://"
	hkdfInfo   = "permaclaw-credential-v1"
	saltLen    = 16
	nonceLen   = 12
	keyLen     = 32
	sshKeyEnv  = "PERMACLAW_SSH_KEY_PATH"
)
...
//
// passphrase is required (PERMACLAW_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// PERMACLAW_SSH_KEY_PATH env var or ~/.ssh/permaclaw_ed25519.key.
...
//   - exact match with PERMACLAW_SSH_KEY_PATH env var
//   - within the PERMACLAW_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with PERMACLAW_SSH_KEY_PATH.
	...
	// Within PERMACLAW_HOME.
	if picoHome := os.Getenv("PERMACLAW_HOME"); picoHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="permaclaw-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set PERMACLAW_SSH_KEY_PATH or place key at ~/.ssh/permaclaw_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (PERMACLAW_SSH_KEY_PATH, PERMACLAW_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. PERMACLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/permaclaw_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the permaclaw-specific SSH key path if it exists.
