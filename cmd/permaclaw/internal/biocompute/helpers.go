...
// ListBiocomputeProcesses prints all biocompute processes owned by the current wallet.
func ListBiocomputeProcesses(client *aomem.MainProcessClient) error {
	ctx := context.Background()
	procs, err := client.ListProcesses(ctx, "biocompute")
	if err != nil {
		return fmt.Errorf("failed to list biocompute processes: %w", err)
	}
	if len(procs) == 0 {
		fmt.Println("No biocompute processes found.")
		return nil
	}
