...
func statusCmd() {
	...
	if cfg.WalletPath != "" {
		fmt.Println("Wallet:", cfg.WalletPath)
	} else {
		fmt.Println("Wallet: not set")
	}
	if cfg.MainProcessID != "" {
		fmt.Println("Main Process ID:", cfg.MainProcessID)
	} else {
		fmt.Println("Main Process ID: not set")
	}

	if _, err := os.Stat(configPath); err == nil {
		fmt.Printf("Model: %s\n", cfg.Agents.Defaults.GetModelName())

		// Legacy provider checks (still valid)
