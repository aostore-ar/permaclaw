...
import (
	...
	"github.com/aostore-ar/permaclaw/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw (openclaw, etc.) to permaclaw",
		Args:  cobra.NoArgs,
		Example: `  permaclaw migrate
  permaclaw migrate --from openclaw
  permaclaw migrate --dry-run
  permaclaw migrate --refresh
  permaclaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.permaclaw)")
