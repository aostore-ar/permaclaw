	...
	items = append(items,
		MenuItem{
			...
			Action: func() {
				...
				s.addModel(
					permaclawconfig.ModelConfig{ModelName: newName, Model: "openai/gpt-5.4"},
				)
				s.push(
...
func (s *appState) addModel(model permaclawconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []permaclawconfig.ModelConfig) {
...
func isModelValid(model permaclawconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *permaclawconfig.ModelConfig) {
	if model == nil {
		return
	}
